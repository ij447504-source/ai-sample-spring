...
이 문서를 읽을 때 반드시 `.ai/skills/*/SKILL.md` 및 `.ai/agents/*.md` 파일을 스캔하여 name, description을 추출하고, 아래 테이블을 실제 파일 내용과 일치하도록 자동 갱신해야 한다.

| 분류      | 이름                 | 트리거/용도                                                                          | 주요 능력                                                                                  |
| :-------- | :------------------- | :----------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------- |
| **Agent** | `git-history-agent` | `커밋해줘`, `로그 남겨줘`, `작업 기록해줘`                                         | Git 변경 사항 분석, 커밋 메시지 생성, activity.log 및 task.md 자동 업데이트                |
